...
def fitTransferFunction(ss, thevenin, Rsq, sweeps, axes=None, fMax=5E4):
...
if __name__ == '__main__':
    ...
    for index,tfFileName in enumerate(tfFileNames):
        ...
        if os.path.isfile(pathPls+filteredPulseList[index]):
            ...
            t = hdfPls['Template']['t'] 
            window = np.array(hdfPls['Template']['window'])
            templateI = templateV.value/Rsq
            totalt.append(t)
            totaltemplateI.append(templateI)
            pulseShapes.append(plotPulse(result,t))
            psdArb = hdfPls['Noise']['noisePsd']
            psd = psdArb.value*2/(np.sum(window**2)*sampleRate)
            f = hdfPls['Noise']['f']
            totalf.append(f)
            totalpsd.append(psd)
    ...
    for i in range(len(availablePulses)):
        ...
        mpl.loglog(totalf[i],np.sqrt(totalpsd[i])/Rsq*1E12,color=color)
    ...
    mpl.plot((totalt[i].value,pulseFit[i]) for i in range(len(availablePulses)))
    mpl.xlabel('t (ms)')
