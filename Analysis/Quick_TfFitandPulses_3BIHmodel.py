...
from Analysis.TesModel_Maasilta import ThreeBlockIHModel
import numpy as np
import lmfit
...
def fitFunction(alphaI, betaI, P, g_tes1, g_tes2, g_2b, Ctes, C1, C2, T, T2, R, betaThermal, f):
    '''Wrapper for complex fits'''
    l = len(f)//2
    omega=2*np.pi*f[:l]
    Z = tesModel.impedance(alphaI, betaI, P, g_tes1, g_tes2, g_2b, Ctes, C1, C2, T, T2, R, betaThermal, omega)
    if not np.all(np.isfinite(Z)):
        print(alphaI, betaI, P, g_tes1, g_tes2, g_2b, Ctes, C1, C2, T, T2, R, betaThermal)
    A = 1./Z
...
def fitTransferFunction(ss, thevenin, Rsq, sweeps, axes=None, fMax=1E5):
    ...
    params['P'].vary      = False; params['P'].value = P0; params['P'].min = 0
    params['g_tes1'].vary = True;  params['g_tes1'].value = 0.8*G; params['g_tes1'].min = 0
    params['g_tes2'].vary = True;  params['g_tes2'].value = 0.1*G; params['g_tes2'].min = 0
    params['g_2b'].vary   = True;  params['g_2b'].value = 0.1*G; params['g_2b'].min = 0
    ...
    params['C2'].vary     = True;  params['C2'].value   = 0.1*C;   params['C2'].min = 0
    params['T'].vary      = False; params['T'].value = T0
    params['T2'].vary     = False; params['T2'].value = T0
    params['R'].vary      = False; params['R'].value = R0
    params['betaThermal'].vary      = False; params['betaThermal'].value = 2.26
    
    
    result = model.fit(data=tesAdmittance.A[iFit].view(dtype=np.float), f=np.hstack([f[iFit], f[iFit]]), params=params )
    ...
    Zfit = tesModel.impedance(alphaI, betaI, P, g_tes1, g_tes2, g_2b, Ctes, C1, C2, T, T2, R, betaThermal, omega)
...
def plotCurrentNoise(psd,result,omega):
    ...
    T = d['T']
    T2 = d['T2']
    noises = tesModel.noiseComponents(alphaI, betaI, P, g_tes1, g_tes2, g_2b, Ctes, C1, C2, T, T2, R, betaThermal, Tshunt,Lsquid, omega)
   

    return noises
    
def plotPulse(result,t):
...
#    Rl = tes.Rshunt 
#    Lsquid = 28.9E-9
#    deltaT = 0.005   #arbitrary at this point
#    tauI = Ctes/(gtesb*(1-L))
#    tauEl = Lsquid/(Rl+R0*(1+betaI))
#    lambdaPlus = 0.5*(1/(tauEl)+1/(tauI) + np.sqrt((1/(tauEl)-1/(tauI))**2 - 4*R0*L*(2+betaI)/(tauTes*Lsquid)))
#    lambdaMinus = 0.5*(1/(tauEl)+1/(tauI) - np.sqrt((1/(tauEl)-1/(tauI))**2 - 4*R0*L*(2+betaI)/(tauTes*Lsquid)))
#    deltaI = (tauI*lambdaPlus-1)*(tauI*lambdaMinus-1)*Ctes*deltaT*(np.exp(np.negative(t)*lambdaPlus)
#    -np.exp(np.negative(t)*lambdaMinus))/((2+betaI)*I0*R0*tauI**2*(lambdaPlus-lambdaMinus))
#    return deltaI
     pass
...
if __name__ == '__main__':
    ...
    if True:
        ...
        tfFileNames80mV = np.array([hdfTf[key].attrs['tfFileName0'].split('/')[3] for key in hdfTf.keys()])
        tfFileNames40mV = np.array([hdfTf[key].attrs['tfFileName1'].split('/')[3] for key in hdfTf.keys()])
        tfFileNames160mV = np.array([hdfTf[key].attrs['tfFileName2'].split('/')[3] for key in hdfTf.keys()])
        pulseFileNames = np.array([hdfTf[key].attrs['pulseFileName'].split('/')[3] for key in hdfTf.keys()])
    ...
    Rn = tes.Rnormal
    Rshunt = tes.Rshunt
    tesModel = ThreeBlockIHModel(Rshunt = Rshunt)
    doPlot = True
    ...
    import pandas as pd
    availablePulses =[]; noises=[]; totalf=[]; totalpsd=[]; pulseBias = []; pulseShapes=[]; totalt=[]; totaltemplateI =[];
    df = pd.DataFrame()
    for index,tfFileName in enumerate(tfFileNames):
        ...
        result, axes =  fitTransferFunction(ss, thevenin, Rsq, sweeps, axes=axes, fMax=1E5)
        if os.path.isfile(pathPls+filteredPulseList[index]):
            ...
            templateV = hdfPls['Template']['templatePulseV']
            t = hdfPls['Template']['t'] 
            templateI = templateV.value/10E3
            totalt.append(t)
            totaltemplateI.append(templateI)
            pulseShapes.append(plotPulse(result,t))
