...
GUI to control the magnet via Avirups magnet box
...
from MagnetSupply3 import MagnetControlThread
...
compileUi('MagnetControl3Ui')
import MagnetControl3Ui
...
class MagnetControlWidget(MagnetControl3Ui.Ui_Widget, QWidget):
    ...
    def startThreads(self):
        ...
        magnetThread.feedbackGainChanged.connect(self.updateFeedbackLeds)
        self.updateFetVsd()
        ...
        # self.updateFeedbackGain(self.feedbackSlider.value())
    ...
    def updateFeedbackLeds(self, gain):
        if gain == 'low':
            self.lowGainLed.turnOn()
            self.highGainLed.turnOff()
        elif gain == 'high':
            self.highGainLed.turnOn()
            self.lowGainLed.turnOff()
        else:
            self.highGainLed.turnOff()
            self.lowGainLed.turnOff()
        
    def updateFeedbackGain(self, value):
        if self.magnetThread is not None:
            gain = {1:'low', 2: 'auto', 3:'high'}[value]
#            self.magnetThread.setFeedbackGain(gain)

        if value in [1,2]: # auto or low
            limit = self.magnetThread.RampRateLimitLowGain / mA_per_min
        else:
            limit = self.magnetThread.RampRateLimitHighGain / mA_per_min
