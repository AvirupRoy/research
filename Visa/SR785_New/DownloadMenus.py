# -*- coding: utf-8 -*-
"""
Dowload all menus and submenus (recursively) of the SR785 display,
saving the screenshots as image files, perform OCR on the menus,
and build up a data-structure with the gathered information
...
sr785 = SR785('GPIB1::10')
menuKeys = SR785.MenuKeyMap

app = gui.QApplication([])

def downloadMenu(name, path):    
    print "Name, path:", name, path
    for keyCode in path:
        sr785.simulateKeyPress(keyCode)

    data = sr785.downloadScreen(sr785.SCREEN_MENU)
    image = gui.QPixmap()
 ...
 #   image = Image.open(BytesIO(data))
    ...
    try:
        ...
        print "Unable to handle image:", e
        return []
    ocrHeader = ocrQImage(analyzer.headerImage())    
    print "Menu header:", ocrHeader
#    headerHash = analyzer.headerHash
    #print "Headings:\n", analyzer.headingHashes
    #print "Values:\n", analyzer.valueHashes
    items = []
    subMenus = []
    for i in range(10):
        ...
        if analyzer.isSubMenu(i) and analyzer.isEnabled(i):
            ...
            sub = downloadMenu(subMenuName, p)
            subMenus.append(sub)
