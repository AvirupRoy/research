...
class MenuImageAnalyzer():
    def __init__(self, image):
        ...
        if w==800 and h == 600:
            ...
            raise Exception('Menu image has unexpected size.')
            
        self.headingHashes = []
        self.valueHashes = []
        for i in range(10):
            self.headingHashes.append(hashQImage(self.itemHeadingImage(i)))
            self.valueHashes.append(hashQImage(self.itemValueImage(i)))
        self.headerHash = hashQImage(self.headerImage())
            
    def headerImage(self):
        header = self.menuImage.copy(QRect(5,5,150,22))
        return header
        
    def itemHeadingImage(self, i):
        heading = self.menuImage.copy(QRect(5,i*57+35, 150, 22))
        return heading
        
    def itemValueImage(self, i):
    ...
    def isEnabled(self, i):
        image = self.itemHeadingImage(i)
        image.save('test__test.png')
        d = []
        for x in range(50):
            c = gui.qGray(image.pixel(x,10))
            d.append(c)  
        i = np.where(np.asarray(d)==0)[0]
        delta = np.diff(i)
        return np.count_nonzero(delta < 2) > 3
#        mpl.figure()
#        mpl.plot(d, '.-')
#        mpl.show()
        
    def isButton(self, i):
        return self.itemType(i) in ['ButtonChecked', 'ButtonUnchecked']
        
    def isSubMenu(self, i):
        return self.itemType(i) in ['SubMenu']
        
    def isEditable(self, i):
        return self.itemType(i)
...
if __name__ == '__main__':
    ...
    for i in range(10):
        print i,
        print analyzer.itemType(i),
        print analyzer.isEnabled(i)
